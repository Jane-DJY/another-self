...
const groups=[
 {field:'科学技术',target:125,cats:['Chinese_women_scientists','American_women_scientists','British_women_scientists','Indian_women_scientists']},
 {field:'商业产品',target:105,cats:['Chinese_businesswomen','American_women_business_executives','British_businesswomen','Indian_businesswomen']},
 {field:'艺术创作',target:151,cats:['Chinese_women_artists','Chinese_women_writers','American_women_artists','British_women_artists','Indian_women_artists']},
 {field:'教育',target:100,cats:['Chinese_women_educators','American_women_educators','British_women_educators','Indian_women_educators']},
 {field:'医疗照护',target:105,cats:['Chinese_women_physicians','American_women_physicians','British_women_medical_doctors','Indian_women_medical_doctors']},
 {field:'公共事务',target:105,cats:['Chinese_women_in_politics','American_women_in_politics','British_women_in_politics','Indian_women_in_politics']},
 {field:'体育探险',target:105,cats:['Chinese_sportswomen','American_sportswomen','British_sportswomen','Indian_sportswomen','Women_explorers']},
 {field:'农业与技能劳动',target:80,cats:['Women_farmers','Women_chefs','Women_artisans','Women_craftworkers']},
 {field:'社区行动',target:100,cats:['Chinese_women_activists','American_women_activists','British_women_activists','Indian_women_activists']}
];
...
if(candidates.length<950)throw new Error(`only ${candidates.length} candidates; expected 950`);
const all=[...existing.map(x=>({...x,editorialStatus:x.editorialStatus||'深度人物卡·已核验'})),...candidates.slice(0,950)];
if(all.length!==1000)throw new Error(`total ${all.length}`);
...
await fs.writeFile('docs/women-stars-candidates.json',JSON.stringify({generatedAt:new Date().toISOString(),policy:'50 deep profiles plus 950 DBpedia and Wikipedia category sourced discovery candidates',stats,candidates:candidates.slice(0,950)},null,2));
console.log(JSON.stringify({total:1000,deep:50,candidates:950,stats},null,2));
